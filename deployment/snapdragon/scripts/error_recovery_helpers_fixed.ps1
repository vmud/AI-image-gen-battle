<#
.SYNOPSIS
    Error Recovery Helper Functions for Snapdragon Deployment (FIXED)
.DESCRIPTION
    ...
    FIXED: Removed Intel-specific packages (numpy-mkl, torch-cpu) that don't exist for ARM64
.NOTES
    Version: 1.1 (Fixed)
    Author: AI Demo Team
...
# ============================================================================
# PACKAGE INSTALLATION FALLBACK (FIXED FOR ARM64)
# ============================================================================

function Get-ProcessorArchitecture {
    <#
    .SYNOPSIS
        Detect processor architecture for proper package selection
    #>
    $arch = $env:PROCESSOR_ARCHITECTURE
    $is_arm = ($arch -eq "ARM64") -or ($arch -eq "ARM")
    
    # Additional check via WMI
    try {
        ...
        Write-VerboseInfo "Could not verify processor via WMI"
    }
...
function Install-PackageWithFallback {
    ...
    foreach ($method in $installMethods) {
        try {
            ...
            $result = & $method -Package $PackageName -Version $Version -ArchInfo $archInfo
            if ($result) {
...
function Install-StandardPackage {
    param($Package, $Version, $ArchInfo)
    
    # Try standard pip install first
    if ($Version) {
        & pip install "$Package$Version" --prefer-binary
    } else {
...
function Install-FromBinaryWheel {
    ...
    # Try to find ARM64 compatible wheels
    if ($ArchInfo.IsARM) {
        # For ARM64, prefer standard PyPI wheels
        $wheelUrl = Get-ARM64WheelUrl -Package $Package -Version $Version
    } else {
...
function Install-AlternativePackage {
    ...
    # FIXED: Architecture-aware alternative packages
    $alternatives = @{}
    
    if ($ArchInfo.IsARM) {
        # ARM64/Snapdragon alternatives
        $alternatives = @{
            "numpy" = "numpy"  # Use standard numpy, not numpy-mkl
            "scipy" = "scipy"  # Use standard scipy, not scipy-mkl
            "torch" = ""       # No direct alternative, will be handled specially
            "tensorflow" = "tensorflow"  # Standard tensorflow
        }
    } else {
        # Intel/AMD64 alternatives
        $alternatives = @{
            "numpy" = "numpy-mkl"
            "scipy" = "scipy-mkl"
            "torch" = "torch-cpu"
    ...
    if ($alternatives.ContainsKey($Package)) {
        ...
        if ($altPackage -eq "") {
            Write-VerboseInfo "No alternative package available for $Package on this architecture"
            return $false
        }
...
function Get-ARM64WheelUrl {
    param($Package, $Version)
    
    # Special handling for ARM64 packages
    $arm64Repos = @(
        "https://www.piwheels.org/simple/$Package/",  # Raspberry Pi wheels (sometimes work)
        "https://pypi.org/simple/$Package/"            # Standard PyPI
    )
    
    foreach ($repo in $arm64Repos) {
        try {
            $response = Invoke-WebRequest -Uri $repo -UseBasicParsing
            # Look for win_arm64 or any wheels
            $wheelLinks = $response.Links | Where-Object { 
                $_.href -match ".*\.whl$" -and 
                ($_.href -match "win_arm64|any|none-any")
            }
            if ($wheelLinks) {
...
# ============================================================================
# PYTORCH SPECIFIC INSTALLATION FOR ARM64
# ============================================================================

function Install-PyTorchForARM64 {
    <#
    .SYNOPSIS
        Install PyTorch specifically for ARM64/Snapdragon systems
    #>
...
"@
                $result = $testScript | python 2>&1
                if ($result -match "PyTorch installation successful") {
                    Write-Success "PyTorch installed successfully via $($method.Name)"
                    return $true
                }
            }
        } catch {
            Write-VerboseInfo "$($method.Name) failed: $_"
        }
    }
    
    Write-WarningMsg "PyTorch installation failed - will proceed with limited functionality"
    return $false
}
...
# ============================================================================
# MODEL DOWNLOAD WITH RESUME (unchanged)
# ============================================================================
...
function Initialize-Logging {
    ...
    Write-Log -Message "Architecture: $env:PROCESSOR_ARCHITECTURE" -Level "Info"
}
